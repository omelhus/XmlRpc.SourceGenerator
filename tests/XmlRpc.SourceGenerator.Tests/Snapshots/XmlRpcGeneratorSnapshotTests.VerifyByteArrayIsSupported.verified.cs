...
public partial class TestXmlRpcClient : ITestXmlRpcClient {
    ...
    public virtual async Task<string> createMethodWithByteArray(byte[] byteArray)
	{
		return await httpClient.SendXmlRpcRequest<string>(Url, "IntegrationManager.createMethodWithByteArray", byteArray).ConfigureAwait(false);
	}
}
